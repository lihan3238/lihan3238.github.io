...
1. 洛必达法则: 若$lim_{x→a}\frac{f(x)}{g(x)}$为$\frac{0}{0}$或$\frac{∞}{∞}$型，且$f(x)与g(x)在a的某去心邻域内可导，且$lim_{x→a}\frac{f'(x)}{g'(x)}$存在，则$lim_{x→a}\frac{f(x)}{g(x)}=lim_{x→a}\frac{f'(x)}{g'(x)}$
...
- 多变出几个式子互相加减
- $\Delta _1 ^ {\Delta _2}$化成$e^{\Delta _2ln\Delta _1}$
